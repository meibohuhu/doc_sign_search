...
from internlm2.modeling_internlm2_alltoken_to_visual import InternLM2Model
...
def extract_attention_with_hook(model, tokenizer, pixel_values, num_patches_list, question, visual_only_attention=False):
    """Extract attention using hook to intercept projector output
    
    Args:
        model: InternVL2 model
        tokenizer: Tokenizer
        pixel_values: Video frames
        num_patches_list: List of patch counts per frame
        question: Question string
        visual_only_attention: If True, only extract visual tokens' self-attention (exclude text tokens)
    """
    ...
    modeling_path = os.path.join(current_dir, 'modeling_internlm2_alltoken_to_visual.py')
...
def process_video(model, tokenizer, video_path, question, args, output_dir, video_file):
    ...
    aggregated_attention, visual_token_idx, generated_text = extract_attention_with_hook(
        ...
        visual_only_attention=args.visual_only_attention
    )
...
def main():
    ...
    parser.add_argument("--visual-only-attention", action="store_true",
                        help="Only extract visual tokens' self-attention (exclude text tokens). Shows how visual tokens attend to each other across different frame regions.")
