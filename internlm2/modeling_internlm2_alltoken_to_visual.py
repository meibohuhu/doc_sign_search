...
class InternLM2Model(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        include_visual_self_attention: bool = False,  # Whether to include visual tokens' self-attention
        visual_only_attention: bool = False,  # If True, only extract visual tokens' self-attention (exclude text tokens)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if output_attentions:
                ...
                if layer_outputs[1].shape[2] != 1:
                    ...
                    # Non-generation case: extract attention to visual tokens
                    ...
                    visual_token_start = visual_token_index[0]
                    visual_token_end = visual_token_index[1] + 1
                    
                    if visual_only_attention:
                        # Only extract visual tokens' self-attention (visual token → visual token)
                        # This shows how visual tokens attend to each other across different frame regions
                        # Query positions: [visual_token_start, ..., visual_token_end-1] (visual tokens only)
                        visual_self_attn = visual_attn[:, :, visual_token_start:visual_token_end, :]
                        # Sum over batch, heads, and visual token query positions only
                        # Result shape: [num_visual_tokens]
                        visual_attn_sum = visual_self_attn.sum(dim=(0, 1, 2))
                    elif include_visual_self_attention:
                        # Include ALL query positions (text tokens + visual tokens themselves)
                        # This gives a complete picture including visual token self-attention
                        # Sum over batch, heads, and ALL query positions (including visual tokens)
                        visual_attn_sum = visual_attn.sum(dim=(0, 1, 2))
                    else:
                        # Only include TEXT token query positions (exclude visual token self-attention)
                        # This avoids double counting and focuses on text→visual attention
                        # Query positions: [0, ..., visual_token_start-1, visual_token_start, ..., visual_token_end-1, visual_token_end, ...]
                        # We want: [0, ..., visual_token_start-1, visual_token_end, ...] (text tokens only)
                        
                        # Extract attention from text tokens before visual tokens
                        text_attn_before = visual_attn[:, :, :visual_token_start, :]
                        # Extract attention from text tokens after visual tokens
                        if visual_token_end < visual_attn.shape[2]:
                            text_attn_after = visual_attn[:, :, visual_token_end:, :]
                            # Concatenate text tokens' attention
                            text_to_visual_attn = torch.cat([text_attn_before, text_attn_after], dim=2)
                        else:
                            text_to_visual_attn = text_attn_before
                        
                        # Sum over batch, heads, and TEXT query positions only (exclude visual token self-attention)
                        visual_attn_sum = text_to_visual_attn.sum(dim=(0, 1, 2))
                else:
                    ...
                    if visual_only_attention:
                        # In generation, query is a text token, so we cannot extract visual token self-attention
                        # Skip this step or use None (will be handled in accumulation)
                        print(f"   ⚠️  visual_only_attention=True in generation case: skipping (query is text token)")
                        visual_attn_sum = None
                    else:
                        # This is from a text token attending to visual tokens
                        visual_attn = layer_outputs[1][:, :, :, visual_token_index[0]:visual_token_index[1]+1]
                ...
                if visual_attn_sum is not None:
...
class InternLM2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            include_visual_self_attention=include_visual_self_attention,
            visual_only_attention=visual_only_attention,
        )
        ...
        model_inputs.update(
            {
                ...
                'include_visual_self_attention': kwargs.get('include_visual_self_attention', False),
                'visual_only_attention': kwargs.get('visual_only_attention', False),
            }
        )
        return model_inputs
