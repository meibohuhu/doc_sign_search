...
# Disable flash attention at the very beginning to avoid compatibility issues
import os
...
## mhu 09/19/2025 comment: it extract frames from video
def extract_frames(video_path, num_frames=8):
    cap = cv2.VideoCapture(video_path)
    frames = []
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    ## Uses np.linspace() to sample 8 frames uniformly
...
############################
    ...
    print(f"[simpleQA] Resolved vision image size: {image_size}")
    ...
    for source in tqdm(data_dict):
        ...
        try:
            if 'video' in source:
                ...
                video_frames = extract_frames(video,32)    ### need to be chagneds
                # if not video_frames:
