...
class ModelArguments:
    ...
    mm_projector_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to pretrained mm_projector.bin"})
    ...
    mm_hidden_size: Optional[int] = field(default=None, metadata={"help": "Hidden size for mm_projector MLP"})
    patch_size: Optional[int] = field(default=16, metadata={"help": "Patch size of the vision tower."})
...
def preprocess_plain(
    ...
    conversations = [] #print these conversations
    for source in sources:
        assert len(source) == 2
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    #########################################################
    ### Added from Vanshika
    ### Flexible Vision Patch Size: custom patch sizes for different vision encoders (MobileCLIP, FastViT, etc.)
    ### Pre-trained Projector loading: 	load existing projector weights from previous training runs
    ...
    if hasattr(model, "vision_tower") and hasattr(model.vision_tower, "config"):
        model.vision_tower.config.patch_size = model_args.patch_size
        print("DEBUG (patch override): patch_size =", model_args.patch_size)
    
    if model_args.mm_projector_path is not None:
        print(f"Loading mm_projector weights from: {model_args.mm_projector_path}")
        mm_projector_weights = torch.load(model_args.mm_projector_path, map_location="cpu")
        
        # Handle key mismatch if needed
        if isinstance(mm_projector_weights, dict) and "model" in mm_projector_weights:
            mm_projector_weights = mm_projector_weights["model"]
        
        # If keys aren't prefixed, manually add the prefix
        if not any(k.startswith("mm_projector.") for k in mm_projector_weights.keys()):
            mm_projector_weights = {f"mm_projector.{k}": v for k, v in mm_projector_weights.items()}
        
        missing_keys, unexpected_keys = model.load_state_dict(mm_projector_weights, strict=False)
        print(f"Loaded mm_projector with missing keys: {missing_keys}, unexpected keys: {unexpected_keys}")
    #########################################################
    return model
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=["v_proj", "up_proj", "down_proj", "k_proj", "o_proj", "gate_proj", "q_proj"],
    ...
    if model_args.vision_tower is not None:
        ...
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
                ...
                assert patch_size in [16, 224, 336, 384, 448, 512, 1024], "patch_size should be in [16, 224, 336, 384, 448, 512, 1024]"
