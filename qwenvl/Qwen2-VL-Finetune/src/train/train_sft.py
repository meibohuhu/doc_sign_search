...
def train():
    ...
    # Check for valid checkpoints (must have trainer_state.json)
    checkpoint_dirs = list(pathlib.Path(training_args.output_dir).glob("checkpoint-*"))
    valid_checkpoints = [cp for cp in checkpoint_dirs if (cp / "trainer_state.json").exists()]
    
    if valid_checkpoints:
        print(f"Found valid checkpoints: {valid_checkpoints}")
        trainer.train(resume_from_checkpoint=True) 
    else:
        print("No valid checkpoints found, starting fresh training")
        trainer.train()  # Start fresh anyway
