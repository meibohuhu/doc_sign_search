...
def eval_model(args):
    ...
    print(f"🎬 Qwen2VL Evaluation on A6000")
    print("=" * 50)
    ...
    for source in tqdm(data_dict, desc="Processing videos"):
        try:
            # Prepare the prompt for ASL translation (keep simple for fine-tuned model)
            fq = "Translate the American Sign Language (ASL) in this video to English."
            if 'video' in source:
    ...
    output_file = f"qwen2vl_a6000_results_{timestamp}.json"
    ...
    if args.enable_evaluation and references and predictions:
        try:
            ...
            # Print results
            print_evaluation_results(eval_results, "Qwen2VL-A6000-Evaluation")
            
            # Save evaluation metrics
            eval_file = os.path.join(args.out_dir, f"evaluation_metrics_{timestamp}.json")
            save_evaluation_results(eval_results, eval_file, "Qwen2VL-A6000-Evaluation")
