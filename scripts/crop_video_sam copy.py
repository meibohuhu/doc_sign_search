...
class SAMVideoProcessor:
    ...
    def detect_face_hands_bboxes(self, rgb_frame):
        """
        Detect and return bounding boxes for face and hands.
        
        Args:
            rgb_frame: RGB frame
            
        Returns:
            dict: Dictionary with 'faces' and 'hands' lists, each containing bbox dicts.
                  Each bbox dict has keys: 'x', 'y', 'width', 'height', 'confidence' (for face)
                  Format: {'faces': [{'x': int, 'y': int, 'width': int, 'height': int, 'confidence': float}, ...],
                          'hands': [{'x': int, 'y': int, 'width': int, 'height': int, 'label': str}, ...]}
        """
        h, w = rgb_frame.shape[:2]
        bboxes = {'faces': [], 'hands': []}
        
        if not self.use_mediapipe:
            raise ValueError("detect_face_hands_bboxes requires MediaPipe. Please install: pip install mediapipe")
        
        # 1. Detect and extract face bboxes
        face_results = self.face_detector.process(rgb_frame)
        if face_results.detections:
            for detection in face_results.detections:
                bbox = detection.location_data.relative_bounding_box
        ...
        # 2. Detect and extract hand bboxes
        hand_results = self.hands.process(rgb_frame)
        if hand_results.multi_hand_landmarks:
            ...
            for idx, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
                ...
                # Get hand label (left/right) if available
    ...
    def process_frame(self, frame, remove_clothing=True, return_bboxes=False):
        """
        ...
        Args:
            ...
            return_bboxes: If True, also return face and hand bounding boxes
            
        Returns:
            tuple: (masked_frame, mask) or (masked_frame, mask, bboxes) if return_bboxes=True
                   bboxes is a dict with 'faces' and 'hands' lists
        """
        # Convert BGR to RGB
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        # Detect face and hand bboxes if requested
        bboxes = None
        if return_bboxes and self.use_mediapipe:
            try:
                bboxes = self.detect_face_hands_bboxes(rgb_frame)
            except Exception as e:
                # If bbox detection fails, return empty bboxes
                bboxes = {'faces': [], 'hands': []}
        ...
        if self.output_size is not None:
            ...
            # Scale bboxes if resizing
            if bboxes is not None:
                scale_w = out_w / frame.shape[1]
                scale_h = out_h / frame.shape[0]
                for face_bbox in bboxes['faces']:
                    face_bbox['x'] = int(face_bbox['x'] * scale_w)
                    face_bbox['y'] = int(face_bbox['y'] * scale_h)
                    face_bbox['width'] = int(face_bbox['width'] * scale_w)
                    face_bbox['height'] = int(face_bbox['height'] * scale_h)
                for hand_bbox in bboxes['hands']:
                    hand_bbox['x'] = int(hand_bbox['x'] * scale_w)
                    hand_bbox['y'] = int(hand_bbox['y'] * scale_h)
                    hand_bbox['width'] = int(hand_bbox['width'] * scale_w)
                    hand_bbox['height'] = int(hand_bbox['height'] * scale_h)
        
        if return_bboxes:
            return masked_frame, mask, bboxes
        return masked_frame, mask
    
    def process_video(self, input_path, output_path, fps=None, save_mask_video=False, save_bboxes=False):
        """
        ...
        Args:
            ...
            save_bboxes: If True, save face and hand bounding boxes to JSON file
        """
        import json
        ...
        # Optional: bbox storage
        all_bboxes = [] if save_bboxes else None
        bbox_output_path = None
        if save_bboxes:
            bbox_output_path = str(output_path).replace('.mp4', '_bboxes.json')
            if not self.use_mediapipe:
                print("⚠️  Warning: save_bboxes requires MediaPipe. Bboxes will not be saved.")
                save_bboxes = False
        ...
        if save_bboxes:
            print(f"📦 Saving bounding boxes to: {bbox_output_path}")
        ...
        with tqdm(total=total_frames, desc="Processing frames", unit="frame") as pbar:
            while cap.isOpened():
                ...
                try:
                    if save_bboxes:
                        masked_frame, mask, bboxes = self.process_frame(
                            frame, 
                            remove_clothing=self.remove_clothing,
                            return_bboxes=True
                        )
                        # Store bboxes with frame number
                        all_bboxes.append({
                            'frame': frame_count,
                            'faces': bboxes['faces'],
                            'hands': bboxes['hands']
        ...
        if save_bboxes and all_bboxes is not None and bbox_output_path:
...
def process_single_video_for_multiprocessing(video_file_path, output_dir, processor_kwargs, fps=None, save_mask_video=False, save_bboxes=False):
    """
    ...
    Args:
        ...
        save_bboxes: Whether to save bounding boxes to JSON file
        
    Returns:
        dict: Status information {'status': 'success'|'skipped'|'error', 'file': filename, ...}
    """
    video_file = Path(video_file_path)
    output_video = Path(output_dir) / f"{video_file.stem}_sam_masked.mp4"
...
def main():
    ...
    parser.add_argument(
        "--save-bboxes",
        action="store_true",
        help="Save face and hand bounding boxes to JSON file (requires MediaPipe). "
             "Output file will be named <output>_bboxes.json with frame-by-frame bbox data."
    )
    parser.add_argument(
        "--fps",
        type=int,
    ...
    # Check save_bboxes requirement
    if args.save_bboxes:
        if not MEDIAPIPE_AVAILABLE:
            print("❌ Error: --save-bboxes requires MediaPipe.")
            print("   Install with: pip install mediapipe")
            sys.exit(1)
