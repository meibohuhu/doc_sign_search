...
Based on qwen25_metrics.py reference with LoRA checkpoint loading
"""
...
def eval_model(args):
    ...
    print(f"🎬 Qwen2VL Checkpoint-5000 Evaluation")
    print("=" * 50)
    ...
    try:
        ...
        # If vision tower was frozen during training, we should NOT load it from checkpoint
        non_lora_path = os.path.join(args.checkpoint_path, 'non_lora_state_dict.bin')
        if os.path.exists(non_lora_path):
            if args.freeze_vision_tower:
                print("🔒 Vision tower was FROZEN during training - keeping original pretrained vision tower")
                print("🔄 Loading only merger weights (excluding vision tower)...")
                non_lora_trainables = torch.load(non_lora_path, map_location='cpu')
                # Clean up state dict keys
                non_lora_trainables = {(k[11:] if k.startswith('base_model.') else k): v for k, v in non_lora_trainables.items()}
                if any(k.startswith('model.model.') for k in non_lora_trainables):
                    non_lora_trainables = {(k[6:] if k.startswith('model.') else k): v for k, v in non_lora_trainables.items()}
                # Filter out vision tower weights - only load merger
                # Vision tower keys may start with: visual., vision_tower., vision_model., etc.
                vision_prefixes = ('visual.', 'vision_tower.', 'vision_model.', 'model.visual.')
                non_lora_trainables_filtered = {k: v for k, v in non_lora_trainables.items() 
                                               if not any(k.startswith(prefix) for prefix in vision_prefixes)}
                excluded_keys = [k for k in non_lora_trainables.keys() 
                                if any(k.startswith(prefix) for prefix in vision_prefixes)]
                excluded_count = len(excluded_keys)
                
                # Log what was excluded for debugging
                if excluded_count > 0:
                    print(f"   Excluded {excluded_count} vision tower parameters:")
                    print(f"   Sample excluded keys: {excluded_keys[:3]}")
                else:
                    print("   ⚠️  Warning: No vision tower parameters found to exclude!")
                    print("   This might indicate the checkpoint doesn't contain vision tower weights,")
                    print("   or the key prefixes don't match expected patterns.")
                
                model.load_state_dict(non_lora_trainables_filtered, strict=False)
                print(f"✅ Loaded {len(non_lora_trainables_filtered)} merger weights")
            else:
                print("🔓 Vision tower was UNFROZEN during training - loading from checkpoint")
                ...
                print(f"   Loading {len(non_lora_trainables)} parameters from checkpoint")
                print(f"   Sample parameter keys: {list(non_lora_trainables.keys())[:3]}")
                
                model.load_state_dict(non_lora_trainables, strict=False)
                print("✅ Loaded non-LoRA weights (vision tower + merger) successfully!")
        else:
            if args.freeze_vision_tower:
                print("✅ Vision tower was frozen - using original pretrained vision tower (no checkpoint needed)")
            else:
    ...
    output_file = f"qwen2vl_checkpoint5000_results_{timestamp}.json"
    ...
    if args.enable_evaluation and references and predictions:
        try:
            ...
            # Print results
            print_evaluation_results(eval_results, "Qwen2VL-Checkpoint-5000")
            
            # Save evaluation metrics
            eval_file = os.path.join(args.out_dir, f"evaluation_metrics_{timestamp}.json")
            save_evaluation_results(eval_results, eval_file, "Qwen2VL-Checkpoint-5000")
...
def main():
    ...
    parser.add_argument("--checkpoint-path", type=str, 
                       default="/shared/rc/llm-gen-agent/mhu/qwen2.5vl/qwen2vl_ssvp_4xa100_12fps/checkpoint-5000",
    ...
    parser.add_argument("--out-dir", type=str, 
                       default="/home/mh2803/projects/sign_language_llm/outputs/",
