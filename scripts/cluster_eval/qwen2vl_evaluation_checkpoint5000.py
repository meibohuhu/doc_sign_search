...
def eval_model(args):
    ...
    for source in tqdm(data_dict, desc="Processing videos"):
        try:
            ...
            # fq = "What do the ASL signs in this video mean?"
            ...
            if 'video' in source:
                ...
                # Use official Qwen2.5-VL video processing pattern - correct format (from reference)
                conversation = [
                    {
                        "role": "user",
                        "content": [
                            {"type": "video", "path": video},  # Note: "path" not "video"
                            {"type": "text", "text": fq},
                        ],
                    }
                ]
                
                # Process video using the official pattern (from reference)
                inputs = processor.apply_chat_template(
                    conversation,
                    video_fps=12,  # Match training fps for consistency
