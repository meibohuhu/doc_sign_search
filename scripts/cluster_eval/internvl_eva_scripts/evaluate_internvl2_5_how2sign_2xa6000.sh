...
# CHECKPOINT_PATH="${CHECKPOINT_PATH:-/local1/mhu/sign_language_llm/InternVL/checkpoints/finetune_internvl2_5_how2sign_16fps_1203/checkpoint-2550}"
...
# VIDEO_FOLDER="${VIDEO_FOLDER:-/local1/mhu/sign_language_llm/openasl/cropped_videos_openasl_test}"
VIDEO_FOLDER="${VIDEO_FOLDER:-/local1/mhu/sign_language_llm/internlm2/test_video}"
# QUESTION_FILE="${QUESTION_FILE:-/local1/mhu/sign_language_llm/InternVL/data/how2sign/test_how2sign_internvl.jsonl}"
QUESTION_FILE="${QUESTION_FILE:-/local1/mhu/sign_language_llm/InternVL/data/testvideo/test_video_internvl.jsonl}"
...
MAX_NUM_FRAMES=${MAX_NUM_FRAMES:-64}  # Maximum number of frames (set to 6 to fix at 6 frames)
SAMPLING_METHOD=${SAMPLING_METHOD:-fps6.0}  # Sampling method: 'fpsX.X' or 'uniform' for uniform sampling
...
$CONDA_PYTHON scripts/cluster_eval/internvl_eva_scripts/internvl_evaluation_how2sign_visualunderstanding.py \
    "${EVAL_ARGS[@]}" \
    2>&1 | tee "$LOG_FILE"
