#!/usr/bin/env python3
"""
InternVL Evaluation Script for how2sign with W&B Logging
Based on internvl_evaluation_how2sign_nogate_nogrpo.py with added wandb support
"""
...
try:
    ...
    HAS_WANDB = False
    print("⚠️  wandb not installed. Run 'pip install wandb' to enable W&B logging.")
...
def eval_model(args):
    ...
    wandb_enabled = HAS_WANDB and not args.no_wandb
    if wandb_enabled:
        try:
            wandb.init(
    ...
    if len(data_dict) == 0:
        ...
        if wandb_enabled:
            wandb.log({"error": "No data loaded"})
            wandb.finish()
        return
    ...
    for idx, source in enumerate(tqdm(data_dict, desc="Evaluating"), 1):
        try:
            ...
            # Log to W&B
            if wandb_enabled and idx % args.wandb_log_frequency == 0:
                wandb.log({
                    "sample_idx": idx,
                    "video": video_file,
                    "prediction": output,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate InternVL trained checkpoint on test set with W&B logging")
    ...
    parser.add_argument("--wandb-run-name", type=str, default=None,
                       help="W&B run name (defaults to checkpoint path or 'eval')")
    parser.add_argument("--wandb-notes", type=str, default="",
                       help="W&B run notes")
    parser.add_argument("--wandb-log-frequency", type=int, default=10,
                       help="Log to W&B every N samples")

    args = parser.parse_args()

    # Set default W&B run name if not provided
    if args.wandb_run_name is None:
        args.wandb_run_name = args.checkpoint_path if args.checkpoint_path else "eval"
