...
cd /mnt/localssd/workspace/doc_sign_search
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-/mnt/localssd/checkpoints/grpo_internvl2_5_how2sign_1b_bleu1_rouge_0314/checkpoint-400}"
...
# CHECKPOINT_PATH="${CHECKPOINT_PATH:-/code/doc_sign_search/script_adobe/checkpoints/finetune_internvl2_5_how2sign_2b_1227_sample15000_notag/checkpoint-3720}"  ## 2744
...
# SFT checkpoint used as PEFT base for GRPO checkpoints (required when CHECKPOINT_PATH is a GRPO DS checkpoint)
SFT_CHECKPOINT="${SFT_CHECKPOINT:-/mnt/localssd/checkpoints/finetune_internvl2_5_how2sign_1b_16fps_1218_121640/checkpoint-2548}"
# MODEL_BASE="${MODEL_BASE:-OpenGVLab/InternVL2_5-1B}"
...
# QUESTION_FILE="${QUESTION_FILE:-/mnt/localssd/workspace/doc_sign_search/InternVL/data/how2sign/test_how2sign_internvl_sample550.jsonl}"

QUESTION_FILE="${QUESTION_FILE:-/mnt/localssd/workspace/doc_sign_search/InternVL/data/how2sign/test_how2sign_internvl.jsonl}"


OUT_DIR="${OUT_DIR:-/mnt/localssd/workspace/doc_sign_search/outputs/internvl_eval/}"

# Evaluation parameters
# MAX_SAMPLES=${MAX_SAMPLES:-20}  # Set to a number to limit samples, empty for full evaluation
MAX_SAMPLES=${MAX_SAMPLES:-2550}
...
# Add SFT checkpoint if provided (used as PEFT base for GRPO checkpoints)
if [ -n "$SFT_CHECKPOINT" ]; then
    EVAL_ARGS+=(--sft-checkpoint "$SFT_CHECKPOINT")
fi
