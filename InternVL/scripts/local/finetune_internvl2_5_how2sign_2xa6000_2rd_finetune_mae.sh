...
# Vision encoder weights from MAE checkpoint (optional)
# Set VISION_ENCODER_WEIGHTS_PATH to load vision encoder weights after model initialization
# This initializes vision encoder from MAE weights but starts training from scratch
# Example: VISION_ENCODER_WEIGHTS_PATH="/local1/mhu/sign_language_llm/InternVL/output/how2sign/internvl2_5_2B_mae_2xa6000/checkpoints/checkpoint-60_vision_encoder/vision_encoder_weights.safetensors"
VISION_ENCODER_WEIGHTS_PATH=${VISION_ENCODER_WEIGHTS_PATH:-"/local1/mhu/sign_language_llm/InternVL/output/how2sign/internvl2_5_2B_mae_2xa6000/checkpoints/checkpoint-60_vision_encoder/vision_encoder_weights.safetensors"}
# VISION_ENCODER_WEIGHTS_PATH=${VISION_ENCODER_WEIGHTS_PATH:-""}
# Resume from checkpoint (optional) - mutually exclusive with VISION_ENCODER_WEIGHTS_PATH
...
# 
# For full checkpoint resume: Use original checkpoint
# Example: RESUME_FROM_CHECKPOINT="/local1/mhu/sign_language_llm/InternVL/checkpoints/internvl2_5_2B_2xa100_mae/checkpoint-30000"
RESUME_FROM_CHECKPOINT=${RESUME_FROM_CHECKPOINT:-""}  # Empty by default (use vision encoder weights instead)  
...
# Note: If resuming from a checkpoint, use the same ZeRO stage as the checkpoint was saved with
# Checkpoint-30000 was saved with ZeRO Stage 3, so we use zero_stage3_config.json
...
elif [ -n "$VISION_ENCODER_WEIGHTS_PATH" ]; then
    echo "Initialize vision encoder from: $VISION_ENCODER_WEIGHTS_PATH"
    echo "   (Training will start from scratch with MAE vision encoder weights)"
else
...
deepspeed --num_gpus=$NUM_DEVICES --master_port=$MASTER_PORT \
    internvl_chat/internvl/train/internvl_chat_finetune_mae.py \
    ...
    $([ -n "$VISION_ENCODER_WEIGHTS_PATH" ] && [ -z "$RESUME_FROM_CHECKPOINT" ] && echo "--vision_encoder_weights_path $VISION_ENCODER_WEIGHTS_PATH") \
    2>&1 | tee "$LOG_FILE"
