...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          ...
                                          InternVLChatConfig,
                                          InternVLChatModel,
                                          InternVLChatModel_vitgateversion)
...
class ModelArguments:
    ...
    use_vit_gate: bool = field(
        default=True,
        metadata={
            'help': 'Enable ViT gated attention mechanism. Default is True.'
        }
    )
    vit_gate_type: Literal['elementwise', 'headwise', 'elementwise_fine'] = field(
        default='elementwise',
        metadata={
            'help': (
                'Type of ViT gate mechanism. Options: '
                "'elementwise' (default: gate shape [B, N, C] applied after reshape), "
                "'headwise' (gate shape [B, N, num_heads, 1] applied before reshape), "
                "'elementwise_fine' (gate shape [B, N, num_heads, head_dim] applied before reshape). "
                'Default is elementwise.'
            )
        },
    )
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        logger.info('Loading InternVLChatModel_vitgateversion...')
        config = InternVLChatConfig.from_pretrained(model_args.model_name_or_path)
        config.vision_config.drop_path_rate = model_args.drop_path_rate
        config.vision_config.use_vit_gate = model_args.use_vit_gate
        config.vision_config.vit_gate_type = model_args.vit_gate_type
        ...
        if config.downsample_ratio != data_args.down_sample_ratio:
            ...
            config.downsample_ratio = data_args.down_sample_ratio
        model = InternVLChatModel_vitgateversion.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config)
    else:
        logger.info('Loading ViT-6B...')
        vision_config = InternVisionConfig.from_pretrained(model_args.vision_path)
        vision_config.drop_path_rate = model_args.drop_path_rate
        vision_config.use_vit_gate = model_args.use_vit_gate
        vision_config.vit_gate_type = model_args.vit_gate_type
        ...
        print('Building InternVLChatModel_vitgateversion...')
        model = InternVLChatModel_vitgateversion(internvl_chat_config, vision_model, llm)
