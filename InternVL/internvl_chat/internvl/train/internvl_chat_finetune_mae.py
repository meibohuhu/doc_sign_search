...
class ModelArguments:
    ...
    vision_encoder_weights_path: Optional[str] = field(
        default=None,
        metadata={'help': 'Path to vision encoder weights file (.safetensors or .pth) to load after model initialization. Useful for loading MAE pretrained vision encoder weights.'}
    )
...
def main():
    ...
    # mhu 2025-11-30: Resize position embedding BEFORE loading vision encoder weights
    # This ensures position_embedding size matches the target image size
    position_embedding_resized = False
    if model.config.vision_config.image_size != data_args.force_image_size:
        ...
        position_embedding_resized = True   ### mhu 2025-11-30: True if position embedding is resized
...
# Layers 2-23: Trainable ✓
    ...
    # mhu 2025-11-30: Load vision encoder weights if specified (e.g., from MAE checkpoint)
    # IMPORTANT: Load weights AFTER freeze/unfreeze logic to preserve requires_grad settings
    if model_args.vision_encoder_weights_path is not None:
        logger.info(f'Loading vision encoder weights from: {model_args.vision_encoder_weights_path}')
        try:
            from safetensors.torch import load_file
            if model_args.vision_encoder_weights_path.endswith('.safetensors'):
                vision_weights = load_file(model_args.vision_encoder_weights_path)
            else:
                vision_weights = torch.load(model_args.vision_encoder_weights_path, map_location='cpu')
            logger.info(f'Loaded {len(vision_weights)} vision encoder weights')
            
            # Remove 'vision_model.' prefix from keys if present
            # When calling model.vision_model.load_state_dict(), keys should not include the 'vision_model.' prefix
            cleaned_vision_weights = {}
            for key, value in vision_weights.items():
                if key.startswith('vision_model.'):
                    # Remove 'vision_model.' prefix
                    new_key = key[len('vision_model.'):]
                    cleaned_vision_weights[new_key] = value
                else:
                    # Key already doesn't have prefix, use as is
                    cleaned_vision_weights[key] = value
            
            logger.info(f'Cleaned weights: {len(cleaned_vision_weights)} keys (removed vision_model. prefix if present)')
            
            # Skip position_embedding if it was resized (size mismatch)
            # Position embedding will be resized based on target image size, so we don't load it from checkpoint
            if position_embedding_resized:
                if 'embeddings.position_embedding' in cleaned_vision_weights:
                    logger.info('Skipping position_embedding (already resized to target image size)')
                    del cleaned_vision_weights['embeddings.position_embedding']
            
            # Store requires_grad state before loading weights
            requires_grad_state = {}
            for name, param in model.vision_model.named_parameters():
                requires_grad_state[name] = param.requires_grad
            
            # Load into model's vision_model (skip size mismatched keys and NaN/Inf values)
            # Filter out keys with size mismatches and NaN/Inf values
            filtered_weights = {}
            model_state = model.vision_model.state_dict()
            shape_mismatches = []
            nan_inf_keys = []
            for key, value in cleaned_vision_weights.items():
                # Check for NaN or Inf values
                if torch.is_tensor(value):
                    if torch.isnan(value).any():
                        nan_inf_keys.append((key, 'NaN'))
                        continue
                    if torch.isinf(value).any():
                        nan_inf_keys.append((key, 'Inf'))
                        continue
                ...
                if key in model_state:
                    if model_state[key].shape == value.shape:
                        ...
                        shape_mismatches.append((key, value.shape, model_state[key].shape))
                        logger.warning(f'Skipping {key}: shape mismatch (checkpoint: {value.shape}, model: {model_state[key].shape})')
                else:
                    # Key not in model, will be reported as unexpected
                    filtered_weights[key] = value
            
            logger.info(f'Filtered weights: {len(filtered_weights)}/{len(cleaned_vision_weights)} keys will be loaded')
            if nan_inf_keys:
                logger.warning(f'Skipped {len(nan_inf_keys)} keys with NaN/Inf values')
                for key, issue_type in nan_inf_keys:
                    logger.warning(f'  {key}: {issue_type}')
            if shape_mismatches:
                logger.warning(f'Skipped {len(shape_mismatches)} keys due to shape mismatches')
            
            missing_keys, unexpected_keys = model.vision_model.load_state_dict(filtered_weights, strict=False)
            
            # Restore requires_grad state after loading weights
            # load_state_dict may reset requires_grad, so we restore it
            restored_count = 0
            for name, param in model.vision_model.named_parameters():
                if name in requires_grad_state:
                    param.requires_grad = requires_grad_state[name]
                    if requires_grad_state[name]:
                        restored_count += 1
            
            logger.info(f'Restored requires_grad for {restored_count} vision_model parameters')
            
            if missing_keys:
                logger.warning(f'Missing keys when loading vision encoder: {len(missing_keys)} keys')
            if unexpected_keys:
                logger.warning(f'Unexpected keys when loading vision encoder: {len(unexpected_keys)} keys')
            
            loaded_count = len(filtered_weights) - len(missing_keys)
            logger.info(f'Vision encoder weights loaded: {loaded_count}/{len(cleaned_vision_weights)} weights successfully loaded')
        except Exception as e:
            logger.error(f'❌ Failed to load vision encoder weights: {e}')
            raise
