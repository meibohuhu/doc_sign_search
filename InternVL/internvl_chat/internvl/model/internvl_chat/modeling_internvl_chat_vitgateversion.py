...
from .modeling_intern_vit_vitgateversion import InternVisionModel, has_flash_attn
...
class InternVLChatModel_vitgateversion(PreTrainedModel):
    ...
    def forward(
        ...
        # 此时 input_embeds 中，IMG_CONTEXT_TOKEN 位置的 embeddings 是文本 token 的 embedding
...
### input_embeds.shape: torch.Size([9295, 2048]), vit_embeds.shape: torch.Size([128, 64, 2048])
    ...
    # 并将这些位置的 embeddings 替换为 vision patch embeddings
        ...
        #            ↑ 标记哪些位置是 image token
        try:    # 步骤 4: 用 patch embeddings 替换 image token 的 embeddings
            ...
            input_embeds[selected] = input_embeds[selected] * 0.0 + vit_embeds[:n_token]  #### selected
            ignore_flag = True
